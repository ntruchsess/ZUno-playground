...
MultiTemperatureSensors multiSensors(&sensors, &temperatureChanged, &addressChanged);

void setup(void)
{
	for (uint8_t index = 0; index < MULTI_TEMPERATURE_SENSORS_MAX_SENSORS; index++)
	{
...
void temperatureChanged(uint8_t index, float temperature)
{
...
void addressChanged(uint8_t index, ssize_t address)
{
	if (index < MULTI_TEMPERATURE_SENSORS_MAX_SENSORS)
	{
